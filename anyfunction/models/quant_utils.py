...
def linear_quantize(input, scale, zero_point, inplace=False, clamp_only=False):
    """
    ...
    if inplace:
        input.mul_(1. / scale).add_(zero_point)
        if not clamp_only:
            input.round_()
        return input

    out = 1. / scale * input + zero_point
    if not clamp_only:
        out = torch.round(out)
    return out
...
class SymmetricQuantFunction(Function):
    ...
    """
    @staticmethod
    def forward(ctx, x, k, percentile_mode=False, specified_scale=None, clamp_only=False):
        """
        ...
        n = 2 ** (k - 1) - 1
        new_quant_x = linear_quantize(x, scale, zero_point, inplace=False, clamp_only=clamp_only)
    ...
    def backward(ctx, grad_output):
        ...
        return grad_output.clone() / scale, None, None, None, None, None
...
class fold_idx_ste(Function):
    """
    Do STE for a specific fold index
    """
    @staticmethod
