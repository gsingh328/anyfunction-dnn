...
class GroupedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0,\
        ...
        # exit(1)
        # self.p = PositionalEncoding(self.folded_in_channels, fold_ratio * 8 + 1, base_freq=10000.)
        # self.p = UniformRandomPositionalEncoding(in_channels, groups + 1)
        self.p = UniformPositionalEncoding(in_channels, groups + 1)
        self.shuffler = ShuffleBlock(self.groups)
    ...
    def forward(self, x):
        ...
        x = self.shuffler(x)
        x = x.reshape(ishape[0], -1, self.groups, ishape[2], ishape[3])
        pe_offset = self.p.pe[0, :self.groups, :self.grouped_in_channels].reshape(1, self.grouped_in_channels, self.groups, 1, 1)

        x = x + pe_offset

        x = x.reshape(-1, self.in_channels, ishape[2], ishape[3])
        x = self.conv(x)
        return x
...
class BasicCNN(nn.Module):
    def __init__(self):
        ...
        self.conv1 = AnyFConv2D(3, 32, 3)

        self.bn1 = nn.BatchNorm2d(32)

        # self.conv2 = nn.Conv2d(32, 64, 3)
        self.conv2 = AnyFConv2D(32, 64, 1)

        self.bn2 = nn.BatchNorm2d(64)
        # self.bn2 = nn.LayerNorm([64, 6, 6], elementwise_affine=True)

        # self.fc1 = nn.Linear(64*6*6, 64)
        # self.fc1 = FoldedAnyFLinear(64*6*6, 64, fold_factor=4, anyf=True)
        self.fc1 = AnyFLinear(64*7*7, 64)

        # self.fc2 = nn.Linear(64, 10)
        self.fc2 = AnyFLinear(64, 10)
